...
MI_INFO *mi_open_share(const char *name, MYISAM_SHARE *old_share, int mode,
  ...
  if (!internal_table) {
    ...
    if (!old_share && !(open_flags & HA_OPEN_FROM_SQL_LAYER)) {
      ...
      if (old_info) old_share = old_info->s;
    }
  }
  ...
  if (!old_share) {
    ...
    if ((kfile = mysql_file_open(mi_key_file_kfile, name_buff,
        ...
        0) {
      if ((errno != EROFS && errno != EACCES) || mode != O_RDONLY ||
    ...
    *strrchr(org_name, '.') = '\0';
    ...
    if (!(open_flags & HA_OPEN_TMP_TABLE)) {
      if ((lock_error = my_lock(
               kfile, F_RDLCK,
               MYF(open_flags & HA_OPEN_WAIT_IF_LOCKED ? 0 : MY_DONT_WAIT))) &&
    ...
    if (len != MI_STATE_INFO_SIZE) {
      DBUG_PRINT("warning",
                 ("saved_state_info_length: %d  state_info_length: %d", len,
                  MI_STATE_INFO_SIZE));
    }
    ...
    max_data_file_length =
        (share->options & (HA_OPTION_PACK_RECORD | HA_OPTION_COMPRESS_RECORD))
            ...
            : (mi_safe_mul(share->base.pack_reclength,
                           (ulonglong)1 << (share->base.rec_reflength * 8)) -
               1);
    max_key_file_length =
    ...
    if (!my_multi_malloc(
            mi_key_memory_MYISAM_SHARE, MY_WME, &share, sizeof(*share),
            &share->state.rec_per_key_part, sizeof(long) * key_parts,
            &share->keyinfo, keys * sizeof(MI_KEYDEF), &share->uniqueinfo,
            uniques * sizeof(MI_UNIQUEDEF), &share->keyparts,
            ...
            &share->mmap_lock, sizeof(mysql_rwlock_t), NullS))
      goto err;
    errpos = 4;
    *share = share_buff;
    memcpy((char *)share->state.rec_per_key_part, (char *)rec_per_key_part,
           sizeof(long) * key_parts);
    memcpy((char *)share->state.key_root, (char *)key_root,
           sizeof(my_off_t) * keys);
    memcpy((char *)share->state.key_del, (char *)key_del,
    ...
    share->blocksize = std::min(ulong{IO_SIZE}, myisam_block_size);
    {
      ...
      for (i = 0; i < keys; i++) {
          ...
          if (pos->type == HA_KEYTYPE_TEXT ||
            ...
            if (!pos->language)
              pos->charset = default_charset_info;
                           ...
                           get_charset(pos->language, MYF(MY_WME)))) {
        ...
        if (share->keyinfo[i].flag & HA_SPATIAL) {
          ...
          if (!fulltext_keys) { /* 4.0 compatibility code, to be removed in 5.0
                                 */
          ...
          if (!share->ft2_keyinfo.seg) {
            ...
            share->ft2_keyinfo.keylength = share->ft2_keyinfo.minlength =
                share->ft2_keyinfo.maxlength =
                    HA_FT_WLEN + share->base.rec_reflength;
      ...
      for (i = 0; i < uniques; i++) {
        ...
        disk_pos_assert(
            disk_pos + share->uniqueinfo[i].keysegs * HA_KEYSEG_SIZE, end_pos);
    ...
    if (mi_open_datafile(&info, share, name, -1)) goto err;
    errpos = 5;
    ...
    if (share->options & HA_OPTION_COMPRESS_RECORD) {
      ...
      info.s = share;
      if (_mi_read_pack_info(
              &info, !(share->options & (HA_OPTION_PACK_RECORD |
    ...
    mysql_mutex_init(mi_key_mutex_MYISAM_SHARE_intern_lock, &share->intern_lock,
                     MY_MUTEX_INIT_FAST);
    for (i = 0; i < keys; i++)
  ...
  if (!my_multi_malloc(
          mi_key_memory_MI_INFO, MY_WME, &m_info, sizeof(MI_INFO), &info.blobs,
          sizeof(MI_BLOB) * share->base.blobs, &info.buff,
          (share->base.max_key_block_length * 2 + share->base.max_key_length),
          &info.lastkey, share->base.max_key_length * 3 + 1,
          &info.rnext_same_key, share->base.max_key_length, &info.first_mbr_key,
          share->base.max_key_length, &info.filename, strlen(name) + 1,
          &info.rtree_recursion_state, share->have_rtree ? 1024 : 0, NullS))
    goto err;
  errpos = 6;
  ...
  */
  ...
  if (!mi_alloc_rec_buff(&info, -1, &info.rec_buff)) goto err;
  ...
  if (!*buf || length > (old_length = mi_get_rec_buff_len(info, *buf))) {
    ...
    extra = ((info->s->options & HA_OPTION_PACK_RECORD)
                 ? ALIGN_SIZE(MI_MAX_DYN_BLOCK_HEADER) + MI_SPLIT_LENGTH +
                       MI_REC_BUFF_OFFSET
                 : 0);
    if (extra && newptr) newptr -= MI_REC_BUFF_OFFSET;
    if (!(newptr =
              (uchar *)my_realloc(mi_key_memory_record_buffer, (uchar *)newptr,
                                  length + extra + 8, MYF(MY_ALLOW_ZERO_PTR))))
      return newptr;
...
static void setup_key_functions(MI_KEYDEF *keyinfo) {
  ...
  if (keyinfo->flag & HA_BINARY_PACK_KEY) { /* Simple prefix compression */
    ...
    if (keyinfo->seg[0].flag & HA_PACK_KEY) { /* Prefix compression */
      /*
...
uint mi_state_info_write(File file, MI_STATE_INFO *state, uint pWrite) {
  ...
  mi_int2store(ptr, state->open_count);
  ptr += 2;
  *ptr++ = (uchar)state->changed;
  *ptr++ = state->sortkey;
  mi_rowstore(ptr, state->state.records);
  ptr += 8;
  mi_rowstore(ptr, state->state.del);
  ptr += 8;
  mi_rowstore(ptr, state->split);
  ptr += 8;
  mi_sizestore(ptr, state->dellink);
  ptr += 8;
  mi_sizestore(ptr, state->state.key_file_length);
  ptr += 8;
  mi_sizestore(ptr, state->state.data_file_length);
  ptr += 8;
  mi_sizestore(ptr, state->state.empty);
  ptr += 8;
  mi_sizestore(ptr, state->state.key_empty);
  ptr += 8;
  mi_int8store(ptr, state->auto_increment);
  ptr += 8;
  mi_int8store(ptr, (ulonglong)state->state.checksum);
  ptr += 8;
  mi_int4store(ptr, state->process);
  ptr += 4;
  mi_int4store(ptr, state->unique);
  ptr += 4;
  mi_int4store(ptr, state->status);
  ptr += 4;
  mi_int4store(ptr, state->update_count);
  ptr += 4;

  ptr += state->state_diff_length;

  for (i = 0; i < keys; i++) {
    mi_sizestore(ptr, state->key_root[i]);
    ptr += 8;
  }
  for (i = 0; i < key_blocks; i++) {
    mi_sizestore(ptr, state->key_del[i]);
    ptr += 8;
  }
  if (pWrite & 2) /* From isamchk */
  {
    ...
    ptr += 4;
    mi_int4store(ptr, state->version);
    ptr += 4;
    mi_int8store(ptr, state->key_map);
    ptr += 8;
    mi_int8store(ptr, (ulonglong)state->create_time);
    ptr += 8;
    mi_int8store(ptr, (ulonglong)state->recover_time);
    ptr += 8;
    mi_int8store(ptr, (ulonglong)state->check_time);
    ptr += 8;
    mi_sizestore(ptr, state->rec_per_key_rows);
    ptr += 8;
    for (i = 0; i < key_parts; i++) {
      mi_int4store(ptr, state->rec_per_key_part[i]);
      ptr += 4;
    }
  }
...
uchar *mi_state_info_read(uchar *ptr, MI_STATE_INFO *state) {
  ...
  state->open_count = mi_uint2korr(ptr);
  ptr += 2;
  state->changed = *ptr++;
  state->sortkey = (uint)*ptr++;
  state->state.records = mi_rowkorr(ptr);
  ptr += 8;
  state->state.del = mi_rowkorr(ptr);
  ptr += 8;
  state->split = mi_rowkorr(ptr);
  ptr += 8;
  state->dellink = mi_sizekorr(ptr);
  ptr += 8;
  state->state.key_file_length = mi_sizekorr(ptr);
  ptr += 8;
  state->state.data_file_length = mi_sizekorr(ptr);
  ptr += 8;
  state->state.empty = mi_sizekorr(ptr);
  ptr += 8;
  state->state.key_empty = mi_sizekorr(ptr);
  ptr += 8;
  state->auto_increment = mi_uint8korr(ptr);
  ptr += 8;
  state->state.checksum = (ha_checksum)mi_uint8korr(ptr);
  ptr += 8;
  state->process = mi_uint4korr(ptr);
  ptr += 4;
  state->unique = mi_uint4korr(ptr);
  ptr += 4;
  state->status = mi_uint4korr(ptr);
  ptr += 4;
  state->update_count = mi_uint4korr(ptr);
  ptr += 4;

  ptr += state->state_diff_length;

  for (i = 0; i < keys; i++) {
    state->key_root[i] = mi_sizekorr(ptr);
    ptr += 8;
  }
  for (i = 0; i < key_blocks; i++) {
    state->key_del[i] = mi_sizekorr(ptr);
    ptr += 8;
  }
  state->sec_index_changed = mi_uint4korr(ptr);
  ptr += 4;
  state->sec_index_used = mi_uint4korr(ptr);
  ptr += 4;
  state->version = mi_uint4korr(ptr);
  ptr += 4;
  state->key_map = mi_uint8korr(ptr);
  ptr += 8;
  state->create_time = (time_t)mi_sizekorr(ptr);
  ptr += 8;
  state->recover_time = (time_t)mi_sizekorr(ptr);
  ptr += 8;
  state->check_time = (time_t)mi_sizekorr(ptr);
  ptr += 8;
  state->rec_per_key_rows = mi_sizekorr(ptr);
  ptr += 8;
  for (i = 0; i < key_parts; i++) {
    state->rec_per_key_part[i] = mi_uint4korr(ptr);
    ptr += 4;
  }
  return ptr;
}
...
uint mi_base_info_write(File file, MI_BASE_INFO *base) {
  ...
  mi_sizestore(ptr, base->keystart);
  ptr += 8;
  mi_sizestore(ptr, base->max_data_file_length);
  ptr += 8;
  mi_sizestore(ptr, base->max_key_file_length);
  ptr += 8;
  mi_rowstore(ptr, base->records);
  ptr += 8;
  mi_rowstore(ptr, base->reloc);
  ptr += 8;
  mi_int4store(ptr, base->mean_row_length);
  ptr += 4;
  mi_int4store(ptr, base->reclength);
  ptr += 4;
  mi_int4store(ptr, base->pack_reclength);
  ptr += 4;
  mi_int4store(ptr, base->min_pack_length);
  ptr += 4;
  mi_int4store(ptr, base->max_pack_length);
  ptr += 4;
  mi_int4store(ptr, base->min_block_length);
  ptr += 4;
  mi_int4store(ptr, base->fields);
  ptr += 4;
  mi_int4store(ptr, base->pack_fields);
  ptr += 4;
  *ptr++ = base->rec_reflength;
  *ptr++ = base->key_reflength;
  *ptr++ = base->keys;
  *ptr++ = base->auto_key;
  mi_int2store(ptr, base->pack_bits);
  ptr += 2;
  mi_int2store(ptr, base->blobs);
  ptr += 2;
  mi_int2store(ptr, base->max_key_block_length);
  ptr += 2;
  mi_int2store(ptr, base->max_key_length);
  ptr += 2;
  mi_int2store(ptr, base->extra_alloc_bytes);
  ptr += 2;
...
uchar *my_n_base_info_read(uchar *ptr, MI_BASE_INFO *base) {
  base->keystart = mi_sizekorr(ptr);
  ptr += 8;
  base->max_data_file_length = mi_sizekorr(ptr);
  ptr += 8;
  base->max_key_file_length = mi_sizekorr(ptr);
  ptr += 8;
  base->records = (ha_rows)mi_sizekorr(ptr);
  ptr += 8;
  base->reloc = (ha_rows)mi_sizekorr(ptr);
  ptr += 8;
  base->mean_row_length = mi_uint4korr(ptr);
  ptr += 4;
  base->reclength = mi_uint4korr(ptr);
  ptr += 4;
  base->pack_reclength = mi_uint4korr(ptr);
  ptr += 4;
  base->min_pack_length = mi_uint4korr(ptr);
  ptr += 4;
  base->max_pack_length = mi_uint4korr(ptr);
  ptr += 4;
  base->min_block_length = mi_uint4korr(ptr);
  ptr += 4;
  base->fields = mi_uint4korr(ptr);
  ptr += 4;
  base->pack_fields = mi_uint4korr(ptr);
  ptr += 4;
  ...
  base->keys = *ptr++;
  base->auto_key = *ptr++;
  base->pack_bits = mi_uint2korr(ptr);
  ptr += 2;
  base->blobs = mi_uint2korr(ptr);
  ptr += 2;
  base->max_key_block_length = mi_uint2korr(ptr);
  ptr += 2;
  base->max_key_length = mi_uint2korr(ptr);
  ptr += 2;
  base->extra_alloc_bytes = mi_uint2korr(ptr);
  ptr += 2;
...
uint mi_keydef_write(File file, MI_KEYDEF *keydef) {
  ...
  *ptr++ = keydef->key_alg; /* Rtree or Btree */
  mi_int2store(ptr, keydef->flag);
  ptr += 2;
  mi_int2store(ptr, keydef->block_length);
  ptr += 2;
  mi_int2store(ptr, keydef->keylength);
  ptr += 2;
  mi_int2store(ptr, keydef->minlength);
  ptr += 2;
  mi_int2store(ptr, keydef->maxlength);
  ptr += 2;
  return mysql_file_write(file, buff, (size_t)(ptr - buff), MYF(MY_NABP)) != 0;
}
...
uchar *mi_keydef_read(uchar *ptr, MI_KEYDEF *keydef) {
  ...
  keydef->flag = mi_uint2korr(ptr);
  ptr += 2;
  keydef->block_length = mi_uint2korr(ptr);
  ptr += 2;
  keydef->keylength = mi_uint2korr(ptr);
  ptr += 2;
  keydef->minlength = mi_uint2korr(ptr);
  ptr += 2;
  keydef->maxlength = mi_uint2korr(ptr);
  ptr += 2;
...
int mi_keyseg_write(File file, const HA_KEYSEG *keyseg) {
  ...
  *ptr++ = keyseg->bit_length;
  mi_int2store(ptr, keyseg->flag);
  ptr += 2;
  mi_int2store(ptr, keyseg->length);
  ptr += 2;
  mi_int4store(ptr, keyseg->start);
  ptr += 4;
...
uchar *mi_keyseg_read(uchar *ptr, HA_KEYSEG *keyseg) {
  ...
  keyseg->flag = mi_uint2korr(ptr);
  ptr += 2;
  keyseg->length = mi_uint2korr(ptr);
  ptr += 2;
  keyseg->start = mi_uint4korr(ptr);
  ptr += 4;
...
uint mi_recinfo_write(File file, MI_COLUMNDEF *recinfo) {
  ...
  mi_int2store(ptr, recinfo->type);
  ptr += 2;
  mi_int2store(ptr, recinfo->length);
  ptr += 2;
  *ptr++ = recinfo->null_bit;
  mi_int2store(ptr, recinfo->null_pos);
  ptr += 2;
  return mysql_file_write(file, buff, (size_t)(ptr - buff), MYF(MY_NABP)) != 0;
}

uchar *mi_recinfo_read(uchar *ptr, MI_COLUMNDEF *recinfo) {
  recinfo->type = mi_sint2korr(ptr);
  ptr += 2;
  recinfo->length = mi_uint2korr(ptr);
  ptr += 2;
  recinfo->null_bit = (uint8)*ptr++;
  recinfo->null_pos = mi_uint2korr(ptr);
  ptr += 2;
  return ptr;
}
...
int mi_open_datafile(MI_INFO *info, MYISAM_SHARE *share, const char *org_name,
  ...
  info->dfile = mysql_file_open(mi_key_file_dfile, data_name,
                                share->mode | O_NOFOLLOW, MYF(MY_WME));
  if (info->dfile < 0) return 1;
...
int mi_open_keyfile(MYISAM_SHARE *share) {
  if ((share->kfile =
           mysql_file_open(mi_key_file_kfile, share->unique_file_name,
                           share->mode, MYF(MY_WME))) < 0)
    return 1;
  return 0;
}
...
int mi_indexes_are_disabled(MI_INFO *info) {
  ...
  */
  ...
  /* All are disabled */
  if (mi_is_any_key_active(share->state.key_map)) return 1;
